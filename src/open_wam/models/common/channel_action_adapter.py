"""Channel-mapped numerical action convention, independent of environment I/O."""
...
class ChannelActionSpec:
    """Channel selection and quantiles defining a model action representation."""
...
class ChannelActionAdapter:
    """Convert channel-mapped actions between raw-space and model-space."""

    def __init__(self, spec: ChannelActionSpec | None) -> None:
        self.spec = spec
    ...
    def source_dim(self) -> int:
        if self.spec is None:
            raise ValueError(
                "Source conversion requires a channel mapping specification."
            )
    ...
    def to_model(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        return self.to_model_action_sequence(
            actions, action_space=ActionSpace.RAW
        ).reshape(
            *actions.shape[:-1],
            self.spec.model_action_dim,
        )

    def to_source(self, actions: torch.Tensor) -> torch.Tensor:
        result = self.to_raw_action_sequence(actions)
        if result is None:
            raise ValueError(
                "Source conversion requires a channel mapping specification."
            )
        return result.reshape(*actions.shape[:-1], self.source_dim)
    ...
    def infer_action_space(self, action: torch.Tensor) -> ActionSpace:
        ...
        if (
            feature_dim == self.spec.model_action_dim
            and feature_dim != self.spec.raw_action_dim
        ):
            return ActionSpace.MODEL
        if (
            feature_dim == self.spec.raw_action_dim
            and feature_dim != self.spec.model_action_dim
        ):
            return ActionSpace.RAW
        if feature_dim == self.spec.model_action_dim == self.spec.raw_action_dim:
            raise ValueError(
                "Automatic channel action-space inference is ambiguous because raw and model action dims are equal. "
                "Pass `action_space='model'` or `action_space='raw'` explicitly."
            )
        raise ValueError(
            "Unable to infer channel action space from the trailing dimension "
    ...
    def to_model_action_sequence(
        ...
        *,
        ...
        if self.spec is None:
            if device is not None or dtype is not None:
                sequence = sequence.to(
                    device=device or sequence.device, dtype=dtype or sequence.dtype
                )
            return sequence
        resolved_space = (
            self.infer_action_space(sequence)
            if action_space == ActionSpace.AUTO
            else ActionSpace(action_space)
        )
        ...
        if resolved_space != ActionSpace.RAW:
            raise ValueError(f"Unsupported channel action_space '{action_space}'.")
        if sequence.shape[-1] != self.spec.raw_action_dim:
            raise ValueError(
                f"Expected raw-space action dim {self.spec.raw_action_dim}, got {sequence.shape[-1]}."
            )
        padded = torch.cat(
            [
                sequence.to(device=target_device, dtype=torch.float32),
                sequence.new_zeros(
                    sequence.shape[0],
                    sequence.shape[1],
                    1,
        ...
        if self.spec.action_norm_method == ActionNormMethod.QUANTILES:
            q01 = torch.tensor(
                self.spec.norm_q01, device=target_device, dtype=torch.float32
            )
            q99 = torch.tensor(
                self.spec.norm_q99, device=target_device, dtype=torch.float32
            )
    ...
    def to_model_action_latents(
        ...
        *,
        ...
        if sequence.shape[1] % action_per_frame != 0:
            raise ValueError(
                "Channel-mapped action sequence length must be divisible by action_per_frame, "
    ...
    def to_raw_action_sequence(
        ...
        if self.spec.action_norm_method == ActionNormMethod.QUANTILES:
            q01 = torch.tensor(
                self.spec.norm_q01, device=sequence.device, dtype=torch.float32
            )
            q99 = torch.tensor(
                self.spec.norm_q99, device=sequence.device, dtype=torch.float32
            )
            sequence = (sequence + 1.0) / 2.0 * (q99 - q01 + 1e-6) + q01
        gather_ids = torch.tensor(
            self.spec.used_action_channel_ids, device=sequence.device, dtype=torch.long
        )
    ...
    def _flatten_to_sequence(self, action: torch.Tensor) -> torch.Tensor:
        ...
        raise ValueError(
            "Unsupported channel action tensor shape. Expected [B,T,C], [B,F,A,C], or [B,C,F,A,1], "
