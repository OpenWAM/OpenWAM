"""Allocation, mutation, reset, and materialization for cache payloads."""
    ...
    for _ in range(max(0, int(num_layers))):
        if (
            ...
            stream_ids = torch.full(
                (total_tokens,), -1, device=device, dtype=torch.long
            )
...
def allocate_slot_pool_slots(
    layer_state: SlotPoolLayerState, key_size: int
) -> torch.Tensor:
...
def next_slot_pool_cache_id(layer_state: SlotPoolLayerState) -> torch.Tensor:
    if layer_state.slot_ids is None or layer_state.slot_mask is None:
        raise ValueError(
            "Slot-pool backend requires initialized `slot_ids` and `slot_mask` tensors."
        )
    if bool(layer_state.slot_mask.any().item()):
        return layer_state.slot_ids[layer_state.slot_mask].max() + 1
    return torch.tensor(
        0, device=layer_state.slot_ids.device, dtype=layer_state.slot_ids.dtype
    )
...
def update_slot_pool_layer_state(
    ...
    *,
    ...
    if layer_state.key is None or layer_state.value is None:
        raise ValueError(
            "Slot-pool backend requires preallocated `key` and `value` tensors."
        )
    if key.ndim != 4 or value.ndim != 4:
        raise ValueError(
            f"Expected slot-pool KV tensors with rank 4, got {tuple(key.shape)} / {tuple(value.shape)}"
        )
...
def restore_slot_pool_slots(
    layer_state: SlotPoolLayerState, slots: torch.Tensor | None
) -> None:
...
def materialize_slot_pool_layer_entry(
    layer_state: SlotPoolLayerState,
) -> AttentionCacheEntry:
    if (
...
def materialize_cache_backend_entries(
    ...
    if payload is None or isinstance(payload, MergedPrefixCachePayload):
        return tuple()
    return tuple(
        materialize_slot_pool_layer_entry(layer_state)
        for layer_state in payload.layer_states
    )
