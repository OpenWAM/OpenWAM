...
class ParallelStreamActionDecoder(ActionDecoder):
    """Final action outputs and losses for the parallel-stream architecture."""
    ...
    def forward_train(self, policy_output: PolicyTrainOutput, batch: PolicyTrainBatch) -> ActionDecoderTrainOutput:
        decoder_payload = require_decoder_artifact_payload(
            ...
            payload_type=ParallelDecoderTrainArtifacts,
        )
        latent_pred = decoder_payload.latent_pred
        train_artifacts = decoder_payload.runtime
        loss_weights = decoder_payload.loss_weights
...
# Deprecated import alias for checkpoint-era integrations.
LingbotParallelActionDecoder = ParallelStreamActionDecoder
