"""Reconcile speculative packed MoT history with executed observations."""
...
def reconcile_mot_observed_history(
    *,
    ...
    runtime_state = (
        ...
        and isinstance(policy_state.variant_state, MoTRuntimeState)
        else None
    )
    if runtime_state is None:
        return PolicyObservedHistoryOutput(
            next_state=policy_state,
            debug={"warmup_skipped": True, "reason": "no_mot_runtime_state"},
        )
    ...
    if inference_window_size <= 0:
        raise ValueError(
            "MoT observed-history window size must be positive, "
    ...
    action_tokens_per_frame = resolve_mot_sequence_actions_per_frame(
        action_horizon=int(action_horizon),
