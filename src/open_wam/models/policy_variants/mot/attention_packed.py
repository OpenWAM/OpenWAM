"""Exact packed video/action attention layouts for MoT execution."""
...
def build_mot_packed_coupling_attention_profile(
    *,
    ...
    if chunk_size_frames <= 0:
        raise ValueError(
            f"M5 packed coupling mask requires positive chunk_size_frames, got {chunk_size_frames}."
        )
...
def build_mot_packed_coupling_attention_mask(
    **kwargs,
    ...
    if profile.self_attention_mask is None:
        raise RuntimeError(
            "M5 packed coupling dense profile did not produce a self-attention mask."
        )
