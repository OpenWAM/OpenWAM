...
class MoTConditioning:
    """Prepare MoT video, text, proprio, and mode conditioning tensors."""
    ...
    def uses_legacy_prefix_contract(self) -> bool:
        return (
            ParallelSequenceContract(self.config.parallel_sequence_contract)
    ...
    def resolve_train_hidden_proprio_context(self, batch: PolicyTrainBatch) -> torch.Tensor | None:
        ...
        if not isinstance(value, torch.Tensor):
            raise ValueError(
                "proprio_context_mode=per_chunk_additive requires M5 per-frame "
    ...
    def context_condition_latent_source(self) -> ParallelContextConditionLatentSource:
