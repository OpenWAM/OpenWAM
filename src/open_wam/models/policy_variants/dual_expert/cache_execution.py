"""Learned DualExpert execution against prefilled video and action caches."""
...
def prefill_video_kv_cache(
    *,
    ...
    through the shared video core (matches parallel-stream shared-backbone behavior).
    """

    if observed_prefix.ndim != 5:
        raise ValueError(
            "DualExpert video prefill expects observed_prefix with shape [B, C, T, H, W], "
    ...
    cache_state = visual_tower.prefill_exact_video_cache(
        ...
        cache_name="dual_expert_video_prefill",
        attention_mask=attention_mask,
        cross_attention_mask=cross_attention_mask,
        detach_cache=detach_cache,
    )
    cache_layers: list[DualExpertVideoLayerCache] = []
    for layer_index, entry in enumerate(cache_state.self_attention_kv):
        if entry.key is None or entry.value is None:
            raise ValueError(
                "DualExpert video prefill expected materialized self-attention K/V entries, "
    ...
    if not cache_layers:
        raise ValueError("DualExpert video prefill did not materialize any layer cache entries.")
    return DualExpertVideoCache(layers=tuple(cache_layers), video_seq_len=int(cache_layers[0].key.shape[2]))
...
def forward_action_with_video_and_action_cache(
    *,
    ...
    action_cache: DualExpertActionCache | None,
    attention_mask: torch.Tensor,
) -> tuple[torch.Tensor, DualExpertActionCache]:
    """Run the action expert against cached video AND cached past-action K/V.

    parallel-stream-aligned variant of `forward_action_with_video_cache`. Past
    ...
    K/V, so the caller can append to the running `DualExpertActionCache` at the
    last denoise step (matching parallel-stream's `update_cache=1` semantics).
    """

    if len(video_cache.layers) != len(action_expert.blocks):
        raise ValueError(
            "DualExpert action runtime requires one cached video K/V pair per action layer, "
    ...
    if action_cache is not None and len(action_cache.layers) != len(action_expert.blocks):
        raise ValueError(
            "DualExpert action runtime requires one cached action K/V pair per action layer, "
    ...
    if attention_mask.shape != (expected_total, expected_total):
        raise ValueError(
            "DualExpert action runtime requires a square joint attention mask matching video+action_cache+fresh length, "
    ...
    fresh_kv_layers: list[DualExpertActionLayerCache] = []
    ...
    fresh_cache = DualExpertActionCache(
...
def forward_action_with_video_cache(
    *,
    ...
    if attention_mask.shape != (expected_total, expected_total):
        raise ValueError(
            "DualExpert action runtime requires a square joint attention mask matching cache+action length, "
            f"got attention_mask={tuple(attention_mask.shape)}, expected=({expected_total}, {expected_total})."
        )
