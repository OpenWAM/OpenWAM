...
class DualExpertPackedInferenceProgram:
    ...
    def run(
        ...
        if (
            _is_dual_expert_generalist_conditional_rollout(generalist_rollout_mode)
        ...
        frame_chunk_size, action_horizon, action_tokens_per_frame = resolve_dual_expert_rollout_frame_chunk_size(
            context,
        ...
        diagnostic_zero_video_noise = bool(context.extra.get("dual_expert_diagnostic_zero_current_video_noise", False))
        diagnostic_zero_action_noise = bool(context.extra.get("dual_expert_diagnostic_zero_current_action_noise", False))
        ...
        if generalist_rollout_mode == GeneralistDenoisingMode.ACTION_CONDITIONED_VIDEO:
            if forced_action_latents is None:  # pragma: no cover - guarded by required=True
                raise RuntimeError("dual-expert FDM rollout missing forced action latents.")
        ...
        elif generalist_rollout_mode == GeneralistDenoisingMode.VIDEO_CONDITIONED_ACTION:
            if video_condition_latents is None:  # pragma: no cover - guarded by required=True
                raise RuntimeError("dual-expert IDM rollout missing video condition latents.")
        ...
        if commit_action_latents is not None and int(commit_action_latents.shape[1]) != self.action_horizon:
            raise ValueError(
                "dual-expert GJD commit action latents must contain the generated action horizon only after coercion, "
        ...
        def _coupled_action_timestep_for_video_step(
            *,
            ...
            if couple_action_video_sigmas:
                ...
                shared_sigma_next = dual_expert_scheduler_next_sigma(video_scheduler, int(step_index)).to(
                    device=device,
                    dtype=torch.float32,
                )
                if joint_timestep_coupling == JointTimestepCoupling.MATCH_SIGMA:
        ...
        if generalist_rollout_mode == GeneralistDenoisingMode.ACTION_CONDITIONED_VIDEO:
            if forced_clean_action_condition is None:  # pragma: no cover - guarded earlier
                raise RuntimeError("dual-expert FDM rollout missing clean action condition.")
        ...
        return PolicyInferOutput(
            ...
            aux={
                ...
                "dual_expert_generalist_rollout_mode": generalist_rollout_mode.value,
                ...
                "dual_expert_invalid_startup_action_tokens": int(current_action_prefix_tokens),
