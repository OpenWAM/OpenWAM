"""Packed-coupling recurrent inference program for DualExpert policies."""
...
from .inference_layout import DualExpertPackedHistory, DualExpertPackedInferenceLayout
...
class DualExpertPackedInferenceProgram:
    """Execute native packed dual-expert rollout without owning model parameters."""
    ...
    def run(
        ...
        if (
            ...
            raise ValueError(
                "dual-expert packed action-only rollout is only used for action_then_video; "
        ...
        explicit_frame_chunk_size = context.extra.get(
            "dual_expert_rollout_frame_chunk_size"
        )
        if (
            explicit_frame_chunk_size is not None
            and int(explicit_frame_chunk_size) != required_frame_chunk_size
        ):
            raise ValueError(
                f"Rollout objective {generalist_rollout_mode.value!r} requires "
                f"frame_chunk_size={required_frame_chunk_size}, but "
                f"{int(explicit_frame_chunk_size)} was requested."
            )
        frame_chunk_size = required_frame_chunk_size
        action_horizon = frame_chunk_size * action_tokens_per_frame
        runtime_state.chunk_advance_frames = int(frame_chunk_size)
        ...
        packed_inference_layout = DualExpertPackedInferenceLayout(
            ...
            current_video_sequence_frames=current_video_prefix_frames
            + frame_chunk_size,
        ...
        packed_history = DualExpertPackedHistory.from_runtime_state(
            runtime_state,
        ...
        if first_step_bootstrap:
            ...
            current_noisy_video = torch.cat(
                [observed_prefix.to(dtype=dtype), current_generated_video], dim=2
            )
            ...
            if current_video_frames >= frame_chunk_size:
                current_video_condition = current_video_observation[
                    :, :, -frame_chunk_size:
                ].contiguous()
            else:
                ...
                current_video_condition = torch.cat(
                    [
                        ...
                        current_video_observation[:, :, -1:].expand(
                            -1, -1, pad_frames, -1, -1
                        ),
                    ],
                    dim=2,
                ).contiguous()
            current_noisy_video = torch.randn_like(
                current_video_condition, device=device, dtype=dtype
            )
            current_clean_video = torch.zeros_like(current_noisy_video)
        current_video_sequence_frames = int(current_noisy_video.shape[2])
        current_action_sample = torch.randn(
            batch_size, action_horizon, self.action_dim, device=device, dtype=dtype
        )
        diagnostic_zero_video_noise = bool(
            context.extra.get("dual_expert_diagnostic_zero_current_video_noise", False)
        )
        diagnostic_zero_action_noise = bool(
            context.extra.get("dual_expert_diagnostic_zero_current_action_noise", False)
        )
        ...
        conditional_inputs = packed_inference_layout.resolve_dynamics_rollout_inputs(
            dynamics_rollout_plan.require_generation_inputs(),
        ...
        if generalist_rollout_mode == DynamicsObjective.ACTION_CONDITIONED_VIDEO:
            if (
                forced_action_latents is None
            ):  # pragma: no cover - guarded by required=True
                raise RuntimeError(
                    "dual-expert FDM rollout missing forced action latents."
                )
        ...
        elif generalist_rollout_mode == DynamicsObjective.VIDEO_CONDITIONED_ACTION:
            if (
                video_condition_latents is None
            ):  # pragma: no cover - guarded by required=True
                raise RuntimeError(
                    "dual-expert IDM rollout missing video condition latents."
                )
            current_noisy_video = video_condition_latents.contiguous()
            current_clean_video = video_condition_latents.contiguous()
            current_video_sequence_frames = int(current_noisy_video.shape[2])
        if (
            commit_action_latents is not None
            and int(commit_action_latents.shape[1]) != action_horizon
        ):
            raise ValueError(
                "dual-expert conditional dynamics action commits must contain "
                "the generated action horizon only after coercion, "
                f"got shape={tuple(commit_action_latents.shape)}, "
                f"action_horizon={action_horizon}."
            )

        history_window_frames = resolve_dual_expert_rollout_cache_window_frames(
        ...
        history_video_timesteps = torch.zeros(
            batch_size, shared_history_frames, device=device, dtype=torch.float32
        )
        ...
        if len(video_scheduler.timesteps) != len(action_scheduler.timesteps):
            raise ValueError(
                "dual-expert packed coupling inference expects matched video/action denoise step counts, "
                f"got video_steps={len(video_scheduler.timesteps)}, action_steps={len(action_scheduler.timesteps)}."
            )
        couple_action_video_sigmas = should_couple_dual_expert_action_to_video_sigmas(
            self.config
        )
        ...
        packed_chunk_origin_frame = startup_plan.chunk_origin_frame(
        ...
        attention_profile = build_dual_expert_packed_coupling_attention_profile(
            ...
            num_action_frames=shared_history_frames
            ...
            history_stream_visibility=rollout_geometry.history_stream_visibility.value,
        ...
        if shared_history_frames > 0:
            ...
            action_sequence_grid_ids = torch.cat(
                [history_action_grid_ids, action_grid_ids], dim=2
            )
        else:
            action_sequence_grid_ids = action_grid_ids
        packed_action_grid_ids = torch.cat(
            [action_sequence_grid_ids, action_sequence_grid_ids], dim=2
        )
        ...
        def _build_packed_action_pre(
            *,
            ...
            if history_actions is None:
                ...
                noisy_action_sequence = torch.cat(
                    [history_actions, action_tokens], dim=1
                )
                ...
                clean_action_sequence = torch.cat(
                    [history_actions, current_clean_action_for_step], dim=1
                )
            packed_action_tokens = torch.cat(
                [noisy_action_sequence, clean_action_sequence], dim=1
            )
        ...
        def _run_packed_step(
            *,
            ...
            dense_video_timestep = torch.cat(
                [history_video_timesteps, video_timestep], dim=1
            )
            ...
            return forward_dual_expert_packed_coupling_denoise(
                visual_tower=visual_tower,
                noisy_video_latents=predicted_video_sequence,
                clean_video_latents=history.prepend_video(current_clean_video_for_step),
        ...
        def _video_timestep(value: torch.Tensor) -> torch.Tensor:
            timestep = expand_dual_expert_scalar_timestep(
                value,
            ...
            if current_video_prefix_frames > 0:
                ...
                predicted_video_sequence[
                    :,
                    :,
                    shared_history_frames : shared_history_frames
                    + current_video_prefix_frames,
        ...
        def _update_video(
            ...
            *,
            ...
            current_video_flow = video_flow_pred[
            ...
            if sigma is None or sigma_next is None:
                current_predicted_video = video_scheduler.step(
                    current_video_flow, video_timestep, current_predicted_video
                )
            else:
                current_predicted_video = step_dual_expert_flow_with_sigmas(
            ...
            predicted_video_sequence = torch.cat(
                [
                    predicted_video_sequence[:, :, :generated_start],
                    current_predicted_video,
                ],
                dim=2,
            )
        ...
        def _update_action(
            ...
            *,
            ...
            action_flow_pred = packed_action_flow[
                :, flow_start : flow_start + action_horizon
            ].contiguous()
            generated_action_timestep = action_timestep[
                :, current_action_prefix_tokens:
            ].contiguous()
            scheduler_timestep = generated_action_timestep.reshape(-1)[0]
            if sigma is None or sigma_next is None:
                action_sample = action_scheduler.step(
                    action_flow_pred, scheduler_timestep, action_sample
                )
            else:
                action_sample = step_dual_expert_flow_with_sigmas(
                    action_sample,
                    action_flow_pred,
                    sigma=sigma,
                    sigma_next=sigma_next,
        ...
        def _coupled_action_timestep_for_video_step(
            *,
            ...
            if couple_action_video_sigmas:
                shared_sigma = video_scheduler.sigmas[int(step_index)].to(
                    device=device, dtype=torch.float32
                )
                shared_sigma_next = dual_expert_scheduler_next_sigma(
                    video_scheduler, int(step_index)
                ).to(
                    device=device,
                    dtype=torch.float32,
                )
                if joint_timestep_coupling == JointTimestepCoupling.MATCH_SIGMA:
                    if (
                        action_timestep_lookup_scheduler is None
                    ):  # pragma: no cover - defensive guard
                        raise RuntimeError(
                            "dual-expert match-sigma same-step inference requires an action timestep lookup scheduler."
                        )
                    ...
                    action_timestep = video_timestep.to(
                        device=device, dtype=torch.float32
                    )
        ...
        if generalist_rollout_mode == DynamicsObjective.ACTION_CONDITIONED_VIDEO:
            if (
                forced_clean_action_condition is None
            ):  # pragma: no cover - guarded earlier
                raise RuntimeError(
                    "dual-expert FDM rollout missing clean action condition."
                )
            ...
            current_clean_video = predicted_video_sequence[
                :, :, shared_history_frames:
            ].contiguous()
            for step_index, video_timestep in enumerate(video_scheduler.timesteps):
                action_timestep, shared_sigma, shared_sigma_next = (
                    _coupled_action_timestep_for_video_step(
            ...
            for action_timestep in action_scheduler.timesteps:
                ...
                _update_action(
                    packed_action_hidden, packed_action_pre, current_action_timestep
                )
            ...
            if not action_only_rollout:
                current_clean_action = (
                    packed_inference_layout.compose_current_action_sequence(
                        action_sample
                    )
                )
                for video_timestep in video_scheduler.timesteps:
            ...
            for step_index, video_timestep in enumerate(video_scheduler.timesteps):
                ...
                current_video_timestep = _video_timestep(video_timestep)
                current_action_timestep = _action_timestep(action_timestep)
                video_flow_pred, packed_action_hidden, packed_action_pre = (
                    _run_packed_step(
                        video_timestep=current_video_timestep,
                        action_timestep=current_action_timestep,
                        current_clean_video_for_step=current_clean_video,
        ...
        if action_only_rollout:
            ...
            next_clean_context = clean_video_sequence[
                :, :, :clean_video_prefix_frames
            ].contiguous()
            ...
            next_clean_context = torch.cat(
                [
                    clean_video_sequence[:, :, :clean_video_prefix_frames],
                    predicted_chunk_latents,
                ],
                dim=2,
            )
            pending_predicted_video_frames = frame_chunk_size
        runtime_state.past_clean_latents = next_clean_context[
        ...
        if video_hidden_proprio_sequence is not None:
            ...
            runtime_state.past_hidden_proprio_states = next_hidden_context[
                :, -history_window_frames:
            ].detach()
        else:
            runtime_state.past_hidden_proprio_states = None
        history_action = (
            action_sample if commit_action_latents is None else commit_action_latents
        )
        if history_actions is None:
            next_clean_actions = history_action
        else:
            next_clean_actions = torch.cat([history_actions, history_action], dim=1)
        max_action_history_tokens = history_window_frames * action_tokens_per_frame
        runtime_state.past_clean_actions = next_clean_actions[
            :, -max_action_history_tokens:
        ].detach()
        runtime_state.next_condition_frame_start = int(
            generation_frame_start + frame_chunk_size
        )
        runtime_state.pending_predicted_video_frames = int(
