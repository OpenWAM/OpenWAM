"""Simultaneous joint-denoise inference program for DualExpert policies."""
...
from .attention_unpacked import build_dual_expert_attention_mask
...
from .rollout_geometry import resolve_dual_expert_rollout_frame_chunk_size


@dataclass(frozen=True)
class DualExpertJointDenoiseInferenceProgram:
    ...
    def run(
        ...
        if _is_dual_expert_generalist_conditional_rollout(generalist_rollout_mode):
            raise ValueError(
                "dual-expert GJD conditional FDM/IDM rollout is implemented for native packed joint coupling, "
                f"not legacy runtime_mode={self.config.runtime_mode!r}."
            )
        if not is_dual_expert_same_step_coupling(current_block_coupling):
            raise NotImplementedError(
                "dual-expert joint_denoise inference supports same-step couplings only; "
        ...
        if action_device != device:
            raise ValueError(
                "DualExpert joint_denoise inference currently requires visual tower and action expert on the same device, "
        ...
        if video_latents.shape[2] <= observed_prefix_frames:
            raise ValueError(
                "DualExpert two-stream inference requires at least one future frame after the observed prefix, "
        ...
        if self.inference_config.video_num_inference_steps != self.inference_config.action_num_inference_steps:
            raise ValueError(
                "DualExpert two-stream inference currently requires matching video/action inference step counts, "
        ...
        attention_mask = build_dual_expert_attention_mask(
        ...
        for step_index, video_timestep in enumerate(video_scheduler.timesteps):
            ...
            if couple_action_video_sigmas:
                ...
                shared_sigma_next = dual_expert_scheduler_next_sigma(video_scheduler, step_index).to(
                    device=device,
                    dtype=torch.float32,
                )
                if joint_timestep_coupling == JointTimestepCoupling.MATCH_SIGMA:
                    if action_timestep_lookup_scheduler is None:  # pragma: no cover - defensive guard
                        raise RuntimeError("dual-expert match-sigma joint denoise requires an action timestep lookup scheduler.")
            ...
            dense_video_timestep = expand_dual_expert_scalar_timestep(
                video_timestep,
                shape=(batch_size, rollout_video_frames),
                device=device,
            )
            dense_video_timestep[:, :observed_prefix_frames] = 0.0
            dense_action_timestep = expand_dual_expert_scalar_timestep(
                action_timestep,
            ...
            if shared_sigma is None or shared_sigma_next is None:
                ...
                noisy_video_latents = step_dual_expert_flow_with_sigmas(
                    noisy_video_latents,
                    video_flow_pred,
                    sigma=shared_sigma,
            ...
            if shared_sigma is None or shared_sigma_next is None:
                ...
                sample = step_dual_expert_flow_with_sigmas(
                    sample,
                    flow_pred,
                    sigma=shared_sigma,
                    sigma_next=shared_sigma_next,
