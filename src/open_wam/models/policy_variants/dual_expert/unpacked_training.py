"""Training programs for DualExpert policies without a packed block stack."""
...
class DualExpertUnpackedTrainingProgram:
    """Execute dual-expert training while video and action blocks retain separate owners."""
    ...
    def _build_video_train_rollout(
        ...
        *,
        ...
        return DualExpertVideoTrainArtifacts(
            flow_pred=flow_pred,
    ...
    def run_prefill_action_denoise(
        ...
        # dual-expert video-prefill action denoise is aligned to parallel-stream full-segment
        ...
        if resolved_text is None:  # pragma: no cover - materialized above
            raise RuntimeError("dual-expert action text context unexpectedly resolved to None.")
        ...
        return PolicyTrainOutput(
            ...
            metrics={
                "dual_expert_history_frames": action_condition_latents.new_tensor(float(history_frames)),
                "dual_expert_video_prefix_frames": action_condition_latents.new_tensor(float(history_frames)),
            },
    ...
    def run_joint_denoise(
        ...
        if not is_dual_expert_same_step_coupling(current_block_coupling):
            raise NotImplementedError(
                "dual-expert joint_denoise train supports same-step couplings only; "
        ...
        if resolved_text is None:  # pragma: no cover - materialized above
            raise RuntimeError("dual-expert joint action text context unexpectedly resolved to None.")
        action_cross_attention_mask = (
            None
            if action_tokens_per_frame is None
            else self.conditioning.build_proprio_cross_attention_mask(
        ...
        decoder_payload = DualExpertTrainArtifacts(
            ...
            video=DualExpertVideoTrainArtifacts(
                flow_pred=video_flow_pred,
                targets=video_artifacts.targets,
                timesteps=video_timesteps,
                scheduler=video_artifacts.scheduler,
