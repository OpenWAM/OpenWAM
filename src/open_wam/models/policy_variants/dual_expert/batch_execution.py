...
from .dual_stream_execution import _video_token_grid_for_latents
...
def _prepare_video(
    visual_tower, request: DualExpertDenoiseRequest
) -> dict[str, torch.Tensor]:
    noisy, clean = request.noisy_video_latents, request.clean_video_latents
    if noisy.shape != clean.shape or noisy.shape[0] != 1:
        raise ValueError(
            "Sequence execution expects individual B=1 samples with matching clean/noisy shape."
        )
    text = request.text_context
    if text is None:
        text = noisy.new_zeros(
            1, visual_tower.config.max_text_tokens, visual_tower.config.text_dim
        )
    clean_time = request.clean_video_timesteps
    if clean_time is None:
        clean_time = torch.zeros_like(request.noisy_video_timesteps)
    grid = build_video_grid_ids(
        _video_token_grid_for_latents(visual_tower, noisy),
        device=noisy.device,
        frame_shift=float(request.frame_start),
    )
    prepared = visual_tower.core.prepare_exact_single_stream_inputs(
        {
            "noisy_latents": torch.cat([noisy, clean], dim=2),
            "text_emb": text.to(device=noisy.device, dtype=noisy.dtype),
            "grid_id": torch.cat([grid, grid], dim=1)[None],
            "timesteps": torch.cat([request.noisy_video_timesteps, clean_time], dim=1),
        },
        action_mode=False,
    )
    if request.video_hidden_context is not None:
        if request.video_hidden_context.shape != prepared["hidden_states"].shape:
            raise ValueError(
                "Sequence video hidden conditioning must match real embedded tokens."
            )
        prepared["hidden_states"] = prepared[
            "hidden_states"
        ] + request.video_hidden_context.to(prepared["hidden_states"])
    return prepared
...
def forward_dual_expert_sequence_batch(
    *,
    ...
    video = [_prepare_video(visual_tower, request) for request in requests]
    ...
    for request, prepared, nv, na, sv, sa in zip(
        ...
        hidden = (
            layer_norm_with_materialized_params(
                visual_tower.core.norm_out, hidden.float()
            )
            * (1.0 + scale)
            + shift
        ).type_as(hidden)
        flow_tokens = linear_with_materialized_params(
            visual_tower.core.proj_out, hidden
        )
        _, _, frames, height, width = request.noisy_video_latents.shape
        flow = unpatchify_video_sequence(
