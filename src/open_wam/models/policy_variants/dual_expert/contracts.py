...
class DualExpertVideoLayerCache:
    key: torch.Tensor
    value: torch.Tensor


@dataclass(frozen=True)
class DualExpertVideoCache:
    layers: tuple[DualExpertVideoLayerCache, ...]
    video_seq_len: int


@dataclass(frozen=True)
class DualExpertActionLayerCache:
    key: torch.Tensor
    value: torch.Tensor


@dataclass(frozen=True)
class DualExpertActionCache:
    layers: tuple[DualExpertActionLayerCache, ...]
    action_seq_len: int


@dataclass
class DualExpertRuntimeState:
    """Typed DualExpert rollout state stored inside `PolicyInferState.variant_state`."""
    ...
    video_cache: DualExpertVideoCache | None = None
    # Persistent per-action-expert-layer K/V cache for past action chunks.
    # Grows by `action_horizon` tokens per chunk when the
    # parallel-stream-aligned non-joint runtime writes the last-step (clean)
    # action K/V back to cache. Mirrors parallel-stream's shared-transformer
    # cache append for action tokens.
    action_cache: DualExpertActionCache | None = None
    ...
    # the parallel-stream-aligned non-joint rollout so each subsequent video
    ...
    # cache, so this tensor provides the action-side continuation that parallel-stream
    ...
    # last packed inference step. Policy-owned observed-history reconciliation
    # replaces exactly this tail with real observations; action-only rollout
    # sets it to zero.
