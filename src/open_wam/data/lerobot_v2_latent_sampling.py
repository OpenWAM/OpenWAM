...
class LocalLatentUniformSegmentSamplingPlan:
    ...
    def build_sample_weights(self) -> tuple[float, ...]:
        ...
        if mode == SampleWeightMode.UNIFORM:
            return tuple(1.0 for _ in self.virtual_index)
        reference_steps = max(
            1.0,
            float(self.dataset_mean_valid_action_steps),
        )
        reference_task_count = max(
            1.0,
            float(self.dataset_mean_task_demo_count),
        )
        weights: list[float] = []
        for virtual_sample_index, (window_index, _) in enumerate(
            self.virtual_index
        ):
            weight = 1.0
            if mode in {
            ...
            if mode in {
                ...
                task_count = max(1, self.task_demo_counts[task_text])
                weight *= reference_task_count / float(task_count)
            if mode == SampleWeightMode.TASK_VIRTUAL_START_COUNT_POWER:
                task_text = self.window_task_texts[window_index]
                task_start_count = max(
                    1.0,
                    float(self.task_virtual_start_counts[task_text]),
                )
                reference_start_count = max(
                    1.0,
                    float(self.dataset_mean_task_virtual_start_count),
                )
                power = float(sample_config.sample_weight_length_power)
                weight *= (
                    task_start_count / reference_start_count
                ) ** (power - 1.0)
