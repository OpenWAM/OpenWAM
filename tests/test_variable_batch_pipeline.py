...
PROGRAMS = (VideoActionProgram.VIDEO_THEN_ACTION, VideoActionProgram.JOINT)


def _tiny_pipeline(program, *, activation_checkpointing=False):
...
def test_variable_batch_matches_independent_single_sample_training(
    ...
    # Collation may harmonize geometry; compare identical sample contracts.
    singles = [
        collate_latent_wam_samples([replace(sample, metadata=batch.metadata[index])])
...
@pytest.mark.parametrize("mode", [BatchingMode.PADDED, BatchingMode.PACKED])
@pytest.mark.parametrize("program", PROGRAMS)
def test_full_segments_draw_one_shared_geometry_per_microbatch(
    mode, program, monkeypatch
):
    pipeline, executor = _tiny_pipeline(program)
    samples = [
        replace(sample, metadata={"sample_index": index, "action_tokens_per_frame": 2})
        for index, sample in enumerate(_samples(program))
    ]
    layout_type = type(pipeline.policy_variant.training_layout)
    original_draw = layout_type.sample_full_segment_geometry
    draws = []

    def draw(layout, **kwargs):
        result = original_draw(layout, **kwargs)
        draws.append(result)
        return result

    monkeypatch.setattr(layout_type, "sample_full_segment_geometry", draw)
    torch.manual_seed(419)

    output = executor.forward_train(_collate(samples, mode))

    assert len(draws) == 1
    assert torch.isfinite(output.loss)
    assert len(output.output.sample_outputs) == 2
    assert (
        len(
            {
                item.policy_output.aux["sampled_chunk_size"]
                for item in output.output.sample_outputs
            }
        )
        == 1
    )
    assert (
        len(
            {
                item.policy_output.aux["sampled_window_size"]
                for item in output.output.sample_outputs
            }
        )
        == 1
...
@pytest.mark.parametrize("program", PROGRAMS)
def test_full_segment_shared_chunk_is_clipped_once_to_shortest_sequence(
    program, monkeypatch
):
    pipeline, executor = _tiny_pipeline(program)
    samples = _samples(program)
    ...
    samples[0] = replace(
        first,
        video_latents=first.video_latents[:, :2],
        actions=first.actions[:4],
        action_mask=first.action_mask[:4],
        condition_latents=first.condition_latents[:, :2],
        proprio_context_frames=first.proprio_context_frames[:2],
        proprio_context_frames_mask=first.proprio_context_frames_mask[:2],
    )
    samples = [
        replace(sample, metadata={"sample_index": index, "action_tokens_per_frame": 2})
        for index, sample in enumerate(samples)
    ...
    monkeypatch.setattr(
        type(pipeline.policy_variant.training_layout),
        "sample_full_segment_geometry",
        lambda *args, **kwargs: (4, 8, 1),
    )

    result = executor.forward_train(_collate(samples, BatchingMode.PACKED))

    assert torch.isfinite(result.loss)
    chunks = [
        item.policy_output.aux["sampled_chunk_size"]
        for item in result.output.sample_outputs
    ]
    assert chunks == [2, 2]
