"""CPU parity + smoke tests for ``DualExpertPackedBlock`` (Step A1).
...
def _make_action_expert() -> DualExpertActionExpert:
    return DualExpertActionExpert(
        hidden_size=_HIDDEN,
        action_dim=4,
        num_layers=_NUM_LAYERS,
        num_heads=_NUM_HEADS,
...
def _packed_block_step_reference(
    ...
    *,
    ...
    new ``DualExpertPackedBlock`` produces the same output.
    """
...
def test_packed_block_forward_parity_with_inline_step() -> None:
    """``DualExpertPackedBlock.forward`` matches the inline ``_packed_block_step`` logic."""
    torch.manual_seed(0)
    ...
    packed = DualExpertPackedBlock(video_block, action_block).eval()
    with torch.no_grad():
...
def test_packed_block_backward_produces_finite_gradients() -> None:
    """Backward through ``DualExpertPackedBlock`` yields finite gradients on both experts.
    ...
    packed = DualExpertPackedBlock(video_block, action_block).train()
...
def test_packed_block_stack_forward_matches_manual_loop() -> None:
    """``DualExpertPackedBlockStack`` running 2 layers matches a manual block-by-block loop."""
    torch.manual_seed(0)
    video_core = _make_video_core()
    action_expert = _make_action_expert()
    ...
    stack = DualExpertPackedBlockStack(video_blocks, action_blocks).eval()
    with torch.no_grad():
...
def test_packed_block_stack_rejects_mismatched_block_counts() -> None:
    ...
    try:
        DualExpertPackedBlockStack(list(video_core.blocks), list(action_expert.blocks)[:1])
...
def test_packed_block_registers_video_and_action_as_children() -> None:
    ...
    video_core = _make_video_core()
    action_expert = _make_action_expert()
    packed = DualExpertPackedBlock(video_core.blocks[0], action_expert.blocks[0])
