...
from .mot_refactor_end_to_end import (
    run_libero_rollout,
    run_training_cli_smoke,
)
from .mot_refactor_fixtures import build_all_characterization_fixtures
...
def verify_characterization(args: argparse.Namespace) -> None:
    ...
    if differences:
        ...
        raise AssertionError(
            f"MoT characterization changed in {len(differences)} places:\n"
            f"{preview}{suffix}"
        )
...
def replay_fixtures(args: argparse.Namespace) -> None:
    ...
    if args.output_root is None:
        with tempfile.TemporaryDirectory(
            prefix="open_wam_mot_fixture_replay_"
        ) as temporary:
            _run_fixture_replay(
                assets=args.assets,
...
def _worker_command(
    *,
    ...
    worker_args = [
        "-m",
        "tests.characterization.mot_refactor_worker",
        "--phase",
        phase,
        "--asset-id",
        asset_id,
    ...
    if isinstance(value, dict):
        ...
        projected = {
            ...
            and not (
                ...
                and _path[-1] not in {"full_training_state.pt", "model_state.pt"}
            )
        }
...
def _selected_asset_ids(raw_ids: list[str] | None) -> tuple[str, ...]:
    ...
    unknown = sorted(set(raw_ids) - set(METHOD_BY_ASSET_ID))
    if unknown:
        raise ValueError(f"Unknown characterization asset ids: {unknown}")
...
def _add_selection_args(
    ...
    *,
    ...
    parser.add_argument(
        "--asset-id",
        action="append",
        choices=tuple(METHOD_BY_ASSET_ID),
        help=(
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Record and verify strict real-checkpoint MoT characterization."
    )
