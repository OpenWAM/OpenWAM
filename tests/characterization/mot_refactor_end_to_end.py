...
from .mot_refactor_artifacts import sha256_file, tensor_fingerprint, write_json_atomic
...
def stage_model_only_checkpoint(source: Path, root: Path) -> Path:
    ...
    if destination.is_symlink() or destination.exists():
        destination.unlink()
    destination.symlink_to(source.expanduser().resolve())
    return destination
...
def build_training_cli_smoke_command(
    *,
    ...
    if method.is_gjd:
        ...
        overrides.update(
            ground_truth_training_overrides(MoTTrainingProfile.FULL_SEGMENT_W64)
        )
    overrides.update(
        {
            "data.num_workers": 0,
...
def build_libero_rollout_command(
    *,
    ...
    if method.is_gjd:
        command.extend(["--mot-generalist-rollout-mode", "joint"])
    return command
...
def _is_libero_summary(value: Any) -> bool:
    return (
        ...
        and value.get("pipeline") == "open_wam_mot"
