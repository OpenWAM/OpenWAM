...
from .mot_refactor_fixtures import CONFIG_ROOT, configure_characterization_paths
from .mot_refactor_provenance import (
            ...
            write_json_atomic(
                output_path,
                {
                    ...
                    "runtime_profile": (
                        "gjd_standard"
                        if method.is_gjd
                        else MoTTrainingProfile.FULL_SEGMENT_W64.value
                    ),
                    "scenarios": reports,
...
def run_inference_characterization(
    *,
    ...
    backend_report = ensure_mot_inference_backend(pipeline, config)
    pipeline.eval()
    ...
    try:
        ...
        if require_cache_rollover:
            payload["cache_rollover_contract"] = {
                ...
                "cache_window_frames": resolve_mot_rollout_cache_window_frames(
...
def _build_inference_characterization_config(*, method: MoTMethodSpec, assets):
    config = load_experiment_config(CONFIG_ROOT / f"{method.config_name}.yaml")
    config = configure_characterization_paths(
        config,
        assets,
...
def _load_pipeline_checkpoint(
    ...
    report = {
        ...
        "trained_missing_keys": [key for key in missing if _is_trained_mot_key(key)],
        "trained_unexpected_keys": [
            key for key in unexpected if _is_trained_mot_key(key)
        ],
    }
    _assert_checkpoint_compatibility(
        report,
    ...
    *,
    ...
    if runtime.strategy.is_main_process:
        ...
        expected_keys = {
            ...
            if _is_trained_mot_key(normalized := _normalize_checkpoint_key(name))
        }
        checkpoint, raw_state, state_dict = _load_checkpoint_state_dict(checkpoint_path)
        checkpoint_keys = set(state_dict)
        missing = sorted(expected_keys - checkpoint_keys)
        unexpected = sorted(
            key for key in checkpoint_keys - expected_keys if _is_trained_mot_key(key)
        )
        report = {
...
def _is_trained_mot_key(key: str) -> bool:
    return key.startswith(TRAINED_MOT_KEY_PREFIXES)
...
def _assert_checkpoint_compatibility(
    ...
    *,
    ...
    if missing:
        details.append(
            f"missing {len(missing)} trained MoT keys: {', '.join(missing[:8])}"
        )
    if unexpected:
        details.append(
            "contains "
            f"{len(unexpected)} unexpected trained MoT keys: "
            f"{', '.join(unexpected[:8])}"
        )
...
def _inference_extra(
    *,
    ...
    if mode == GJDTrainingMode.FDM:
        extra["mot_forced_action_latents"] = action_conditioning
        extra["mot_commit_action_latents"] = action_conditioning
    elif mode == GJDTrainingMode.IDM:
        extra["mot_video_condition_latents"] = video_conditioning
        extra["mot_commit_action_latents"] = action_conditioning
    return extra
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one real-checkpoint MoT characterization worker."
    )
    parser.add_argument(
        "--phase",
    ...
    parser.add_argument("--asset-id", choices=tuple(METHOD_BY_ASSET_ID), required=True)
