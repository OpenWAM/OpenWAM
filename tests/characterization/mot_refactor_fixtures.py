...
def build_all_characterization_fixtures(
    *,
    ...
    for profile, fixture_id in (
        (MoTTrainingProfile.RANDOM_SEGMENT, "mot_random_segment"),
        (MoTTrainingProfile.FULL_SEGMENT_W64, "mot_full_segment_w64"),
    ):
...
def _build_rollout_fixture(
    *,
    ...
    config = _load_non_gjd_fixture_config(
        assets,
        profile=MoTTrainingProfile.FULL_SEGMENT_W64,
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze strict MoT/GJD characterization inputs from real LIBERO data."
    )
