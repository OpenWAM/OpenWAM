...
def test_shared_probability_helpers_cover_m1_and_m5_mode_enums() -> None:
    m1_probs = default_video_action_conditioning_mode_probs(GeneralistDenoisingMode, generalist=True)
    ...
    assert m1_probs[GeneralistDenoisingMode.JOINT] == pytest.approx(0.6)
    assert m5_probs[GeneralistDenoisingMode.JOINT] == pytest.approx(0.6)
    ...
    assert any("Invalid GeneralistDenoisingMode" in issue.message for issue in issues)
    assert any("numeric probability" in issue.message for issue in issues)

    with pytest.raises(ValueError, match="generalist_denoising_mode_probs.*invalid mode"):
        coerce_probability_map(
            {"typo": 1.0},
            enum_cls=GeneralistDenoisingMode,
            field_name="generalist_denoising_mode_probs",
...
def test_shared_conditioning_mode_sampling_broadcasts_rank_zero_choice(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mode = sample_conditioning_mode(
        {mode: 1.0 for mode in GeneralistDenoisingMode},
        enum_cls=GeneralistDenoisingMode,
        device=torch.device("cpu"),
        error_label="test mode",
    )

    assert mode == tuple(GeneralistDenoisingMode)[2]
