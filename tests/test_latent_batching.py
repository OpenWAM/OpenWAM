...
def test_padding_preserves_content_lengths_and_masks(mode):
    ...
    assert batch.metadata[1]["sampled_window_size"] == 4
    assert batch.metadata[1]["history_frames"] == 4
    assert batch.metadata[1]["batching_original_geometry"]["sampled_chunk_size"] == 3
...
def test_collation_rejects_incompatible_fields():
    ...
    with pytest.raises(ValueError, match="Inconsistent optional"):
        collate([_sample(4), replace(_sample(6), condition_latents=None)])
    with pytest.raises(ValueError, match="action rows"):
        collate([_sample(4), replace(_sample(6), actions=torch.ones(13, 3))])
    with pytest.raises(ValueError, match="online canonical RGB"):
        collate([replace(_sample(4), canonical_video=torch.zeros(3, 4, 2, 2))])
    with pytest.raises(ValueError, match="mix explicit"):
        collate([_sample(4), replace(_sample(6), metadata={})])
...
def test_non_strict_loader_rejects_unsupported_sources():
    data = GenericDataConfig(batching={"mode": "padded"})
    config = SimpleNamespace(
        data=data, trainer=SimpleNamespace(batch_adapter=BatchAdapterName.LATENTS)
    )
    with pytest.raises(ValueError, match="local LeRobot"):
        _validate_batching_source(config)
    data = replace(
        data,
        dataset_type="lerobot_v2_latent_local",
        sample_construction=replace(
            data.sample_construction, mode=WindowSamplingMode.UNIFORM_SEGMENT
        ),
    )
    _validate_batching_source(SimpleNamespace(data=data, trainer=config.trainer))
...
def test_geometry_sharing_does_not_consume_rng_or_modify_loss_boundaries():
