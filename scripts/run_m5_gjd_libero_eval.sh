...
eval path, which does not preserve the current M5 GJD rollout contract.

Use the unified GJD rollout path instead:

  bash scripts/run_gjd_libero.sh rollout --method m5 --ablation vanilla \
    --checkpoint /path/to/checkpoint_step_N ...

Valid ablations are vanilla, pure_joint, and mode_token.

For multi-episode M5 GJD eval, use a loop or a future GJD-aware batch wrapper
...
Use scripts/run_gjd_libero.sh rollout --method m5 --ablation <vanilla|pure_joint|mode_token> instead.
EOF
exit 2
