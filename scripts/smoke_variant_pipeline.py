...
from open_wam.models.policy_variants import PolicyInferContext, PolicyTrainBatch, PostLatentPolicyVariant  # noqa: E402
...
def main() -> None:
    ...
    pipeline = VariantPipeline(
        visual_tower=VisualTower(
            LingbotCompatibleVideoBackboneConfig(hidden_size=256, num_layers=1, num_heads=8)
        ),
        policy_variant=PostLatentPolicyVariant(
            config=PostLatentPolicyConfig(hidden_size=256, attach_site="post_visual_core"),
            training_config=training_config,
            inference_config=inference_config,
            action_horizon=data_config.action_schema.action_horizon,
            state_dim=data_config.action_schema.state_dim,
        ),
        action_decoder=MLPActionDecoder(
            hidden_size=MLPActionDecoderConfig(hidden_size=256, action_dim=30, action_horizon=6).hidden_size,
            action_dim=30,
            action_horizon=6,
        ),
    ...
    print("train.video_tokens", tuple(train_output.visual_outputs.core.tokens.shape))  # type: ignore[union-attr]
