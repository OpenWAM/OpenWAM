...
def _check_static_source_contracts() -> None:
    ...
    writer_source = (REPO_ROOT / "deployment/shared/openwam/lerobot_writer.py").read_text(encoding="utf-8")
    if "import cv2" not in writer_source:
        raise SystemExit("deployment/shared/openwam/lerobot_writer.py should lazily import cv2 in the video writer.")
    if "import cv2" in "\n".join(writer_source.splitlines()[:60]):
        raise SystemExit("deployment/shared/openwam/lerobot_writer.py must not import cv2 at module import time.")
