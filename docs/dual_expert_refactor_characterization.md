# DualExpert Refactor Characterization

The DualExpert characterization suite is an opt-in numerical regression gate for
...
The static matrix covers the six maintained non-GJD dual-expert programs:
...
- dual-expert joint and mode-token GJD joint inference run 17 recurrent chunks to cross
...
- the checkpoint has every current trained DualExpert parameter and no stale trained
  DualExpert parameters
...
configured DualExpert inference backend, and runs the streaming frontend plus three
...
`tests/characterization/dual_expert_assets.example.yaml`. The manifest separates:
...
Public manifests and CLI selections use `dual_expert_*` program IDs. The
immutable schema-v1 fixtures and goldens were recorded before that public
naming cleanup, so their on-disk filenames, `asset_id` values, and rollout
fixture discriminator retain `mot_*`. The characterization boundary resolves
public IDs to those frozen artifact IDs; it never renames or regenerates an
existing oracle. Golden comparison similarly canonicalizes only the enumerated
config, provenance-field, backend, and cache-type labels. Tensor hashes,
shapes, probes, losses, gradients, optimizer updates, and rollout values remain
...
uv run python -m tests.characterization.run_dual_expert_refactor_characterization \
  fixtures \
  --assets /path/to/dual_expert_assets.yaml \
...
uv run python -m tests.characterization.run_dual_expert_refactor_characterization \
  ...
  --stage-root /local/nvme/openwam_dual_expert_stage \
...
```bash
OPEN_WAM_RUN_DUAL_EXPERT_GPU_CHARACTERIZATION=1 \
OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_ASSETS=/path/to/dual_expert_assets.yaml \
OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_FIXTURES=/path/to/frozen_fixtures \
OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_GOLDENS=/path/to/golden_reports \
OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_ASSET_ID=dual_expert_joint \
uv run pytest -q tests/test_dual_expert_refactor_gpu_characterization.py
```

Set `OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_STAGE_ROOT` for local staging and
`OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_DISABLE_NCCL_SHM=1` on affected hosts. The
...
continuation rather than duplicating program-specific gradient coverage. Point
...
```bash
OPEN_WAM_RUN_DUAL_EXPERT_CACHE_ROLLOVER=1 \
OPEN_WAM_RUN_DUAL_EXPERT_FULL_STATE_RESUME=1 \
OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_ASSETS=/path/to/dual_expert_assets.yaml \
OPEN_WAM_DUAL_EXPERT_CHARACTERIZATION_FIXTURES=/path/to/frozen_fixtures \
OPEN_WAM_DUAL_EXPERT_INFRASTRUCTURE_GOLDENS=/path/to/infrastructure_goldens \
uv run pytest -q tests/test_dual_expert_refactor_gpu_characterization.py
```
...
Run one full LIBERO rollout for each of the six checkpoint-backed programs,
...
Non-GJD programs retain 800/50 limits and GJD retains 1500/100. Each report
...
episode execution to `open_wam.evals.libero_dual_expert_rollout`. Prepared visual
